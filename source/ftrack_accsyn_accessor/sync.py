...
def on_sync_to_destination(
    session, source_id, destination_id, components, user_id
):
    ...
    for component in components:
        ...
        destination_available = (
            destination_location.get_component_availability(component)
        )
        ...
        if source_available == 0.0:
            ...
            job['data'] = json.dumps({'description': status})
            job['status'] = 'failed'
            session.commit()
            continue
        else:
        ...
        try:
            ...
            logger.error(
                'Component "{}" with ID {} failed: {}'.format(
                    component_name, component_id, error
                )
            )
            import traceback
    ...
    *selection* : a list of the ids of the selected entity in ftrack.
    ...
    for s in selection:
        ...
        for component in version['components']:
            component_name = component['name']
            component_id = component['id']
            components.append({'id': component_id, 'name': component_name})

            status = 'Syncing {} from {} to {}'.format(
                component_name, source_name, sync_name
            )
    ...
    event = ftrack_api.event.base.Event(
        topic='ftrack.sync',
        data={
            'actionIdentifier': 'ftrack-to-{}'.format(
                results['output']['name']
            ),
