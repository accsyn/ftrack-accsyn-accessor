...
from ftrack_accsyn_accessor import sync


logger = logging.getLogger('ftrack_accsyn_accessor.SyncAction')
...
class SyncAction(BaseAction):
    ...
    def get_locations_menu(
        ...
        if exclude_self:
            locations = [
                x for x in locations if not x['name'] == self.location['name']
            ]

        # filter out ftrack locations from sync
        locations = [
            x for x in locations if x['name'] not in self._ignored_locations
        ]
    ...
    def sync_there(self, event):
        try:
            ...
            self._location_data.pop(
                _id
            ) if _id in self._location_data else None
    ...
    def launch(self, session, entities, event):
        self.logger.info(
            "Sync action launched from location {}".format(
                self.location['name']
            )
        )
        ...
        if 'values' not in event['data']:
            ...
            return {'success': True, 'message': 'Sync launched'}
    ...
    def _register(self, event):
        ...
        self.session.event_hub.subscribe(
            ...
            ' and data.location="{1}"'.format(
                self.identifier, self.location['name']
            ),
            self._launch,
        )
        ...
        for location in self.get_locations():
            if location.accessor:
                ...
                self.session.event_hub.subscribe(
                    'data.actionIdentifier={0}-to-ftrack'.format(
                        location['name']
                    ),
                    self.sync_there,
                )
